...
| O — Perception adapter interfaces | `IDepthBackend`, `ISegmentationBackend` (etc.) so future model integration is isolated, mirroring `IPhysicsBackend`/`IReconstructionBackend` (spec §8/§11) | SCAFFOLDED | `perception/depth/interface.py` (`IDepthBackend`, `DepthMap` -- row-validated, no numpy dependency) and `perception/segmentation/interface.py` (`ISegmentationBackend`, `SegmentedRegion`/`SegmentationResult` -- confidence-bounded, carries `model_name`/`model_version` for provenance per spec §48). Zero concrete backends implement either interface -- no depth model, no SAM2, nothing installed or adapted. 10 tests exercise the interface contract itself (ABC enforcement, shape/bounds validation) via minimal fakes, the same pattern `test_colmap_backend.py` uses for `IReconstructionBackend`'s honest-unavailability path. This is explicitly SCAFFOLDED, not PARTIAL: an interface with no implementation is a real, useful artifact (it lets future model integration be isolated and licensing-safe from day one) but it is not perception capability. |
