...
| Depth | IMPLEMENTED (real, metricized) | `perception/depth/midAS_backend.py` (MiDaS via torch.hub, honest relative `unit`) + `reconstruction/depth_to_points.py` (per-view metricization against the SfM sparse cloud — labeled **metric-by-alignment**, approximate) + pipeline stage 2.8. Real-model verified: MiDaS_small CPU 4.1 s/map; DPT_Hybrid ~33–41 mm residuals on the fixture. |
| Segmentation | IMPLEMENTED (real-model verified) | `perception/detection/maskrcnn_backend.py` — COCO Mask R-CNN implements `ISegmentationBackend` (source-resolution instance masks, verified on a real photograph) alongside `IDetectorBackend`. The SAM backend remains available for point-prompted use but its hub cache is broken in this environment (pre-existing). |
| Detection | IMPLEMENTED (real-model verified) | `perception/detection/maskrcnn_backend.py` + `perception/detection/interface.py` (`Detection`, `IDetectorBackend`) — real COCO-pretrained detector with score threshold, model provenance, honest unavailability. Previously an empty README scaffold. Verified: real photo → `book` 0.78 / `person` 0.51; synthetic noise fixture → 0 detections (honest OOD boundary, asserted in tests). |
| Point cloud / mesh generation | PARTIAL (dense points, no mesh) | Pipeline stage 2.8 fuses metricized depth into a dense world-frame point cloud (97k–121k points on the fixture). No TSDF/Poisson surface reconstruction exists — `Geometry` still stores only `vertex_count`, never real vertex buffers (unchanged from every prior audit). |
| 2D→3D lifting / object hypotheses | IMPLEMENTED (INTEGRATED this campaign) | `perception/instances/lifting.py` (mask+metric-depth+camera → `ObjectHypothesis3D`, refuses relative depth, honest None) existed but was UNWIRED; this campaign wired it: pipeline stage 3.5 (segment → lift → `merge_hypotheses` → `promote_object_to_entity` → validated WorldIR). Traceable per-entity evidence ids. |
