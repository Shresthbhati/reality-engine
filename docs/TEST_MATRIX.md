...
(438 -> 488 -> 479 on separate unmerged branches -> hundreds more from
evidence/reconstruction/studio/perception work merged since). Counts left
as "—" above were not individually re-verified in this pass; re-run
`pytest --collect-only -q` per file to refresh, and prefer
`docs/CAPABILITY_MATRIX.md` for what's actually tested per subsystem
going forward.
